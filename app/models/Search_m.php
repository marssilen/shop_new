...
class Search_m extends Model
{
	...
	function getCatItems($sql,$page,$rows=100){
//        return $this->db->select($sql);
        return $this->db->pagination($sql, array(),$page,$rows);
    }
    function get_pview($cat,$sql,$numrows,$rows_per_page=100){
        $pages= ceil($numrows/$rows_per_page);
        return create_pview(URL.'search/'.$cat,$pages,$this->get_cat_name($cat));
    }
